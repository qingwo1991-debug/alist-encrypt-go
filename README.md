...
列表中出现 `orig_xxx.bin`（前缀加一串密文文件名）**不代表该文件内容解密失败**。它是显示层标记：一个加密文件的**文件名自身解码不出明文展示名**（CRC6 不匹配或 MixBase64 解码失败），常见于从其它实例/密码体系拷贝过来的文件——内容密文仍正常解密，只是列表里丢失了友好的明文文件名。

具体行为：

- 播放/下载入口（`/d/*`、`/p/*`、`/dav/*`、`/redirect?decode=1`）仍按内容头探测并正常解密，返回明文。
- `orig_` 前缀只影响展示名和“名字→路径”的映射启发（这些启发会跳过 `orig_` 文件），不影响内容本身。
- 若想恢复明文文件名，请把文件放到与该明文名同一密码/加密类型下重新落位（重新加密命名），或仅在目标实例明文名可解码时才拷贝。
