...
func TestHandleDownloadUsesWarmCacheWithoutMetadataPrefetch(t *testing.T) {
	cfg := config.Get()
	original := cfg.AlistServer
	t.Cleanup(func() {
		cfg.AlistServer = original
	...
	var fsGetCalls, fsLinkCalls, rawCalls int
	var backendURL string
	backend := newSocketTestServer(t, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/api/fs/get":
			fsGetCalls++
			t.Fatalf("fs/get should not be called when warm cache is present")
		case "/api/fs/link":
			fsLinkCalls++
			t.Fatalf("fs/link should not be called when warm cache is present")
		case "/raw/movie.mp4":
			rawCalls++
			w.Header().Set("Content-Type", "video/mp4")
			w.Header().Set("Content-Length", strconv.FormatInt(fileSize, 10))
	...
	if fsGetCalls != 0 || fsLinkCalls != 0 {
		t.Fatalf("metadata prefetch should not run, fs/get=%d fs/link=%d", fsGetCalls, fsLinkCalls)
	}
	if rawCalls != 1 {
		t.Fatalf("raw calls=%d, want 1", rawCalls)
	}
}

func TestHandleDownloadRefreshesStaleWarmCache(t *testing.T) {
	cfg := config.Get()
	original := cfg.AlistServer
	t.Cleanup(func() {
		cfg.AlistServer = original
	...
	backend := newSocketTestServer(t, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/api/fs/link":
			fsLinkCalls++
			t.Fatalf("fs/link should not be needed when fs/get returns useful metadata")
		case "/raw/movie.mp4":
			w.Header().Set("Content-Type", "video/mp4")
			w.Header().Set("Content-Length", strconv.FormatInt(fileSize, 10))
			w.WriteHeader(http.StatusOK)
	...
	if fsGetCalls == 0 {
		t.Fatal("expected fs/get refresh for stale cache")
	}
	if fsLinkCalls != 0 {
		t.Fatalf("fs/link calls=%d, want 0", fsLinkCalls)
	}
	info, ok := fileDAO.Get("/encrypt/movie.mp4")
	if !ok || info == nil {
		t.Fatal("expected cached file info after refresh")
	}
	if info.RawURL != backendURL+"/raw/movie.mp4" {
		t.Fatalf("raw_url=%q, want refreshed value", info.RawURL)
	}
}
