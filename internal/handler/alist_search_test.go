...
func TestHandleFsListRejectsInvalidBackgroundSnapshot(t *testing.T) {
	...
	scanScopeKey := buildDirScopeKey("/移动云盘192", dirSyncScopeScan)
	err = dirStore.UpsertSnapshot(context.Background(), DirListSnapshot{
		...
		PayloadJSON:   []byte(`{"code":200,"message":"success","data":{"content":[{"name":"错误目录","path":"/豆包云/错误目录","is_dir":true,"size":0,"type":1}],"total":1}}`),
	})
	if err != nil {
		t.Fatalf("seed scan snapshot: %v", err)
	}

	reqBody := `{"path":"/移动云盘192","page":1,"per_page":1000,"refresh":false}`
	...
	if got, _ := resp.Data.Content[0]["path"].(string); got != "/移动云盘192/正确目录" {
