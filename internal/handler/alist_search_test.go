...
func TestHandleFsListRejectsRequestSnapshotWithoutPathsForNonRootDir(t *testing.T) {
	...
	reqBody := `{"path":"/user_storage","page":1,"per_page":1000,"refresh":false}`
	authHash := authScopeHash(handler.requestAuthHeaders(httptest.NewRequest(http.MethodPost, "/api/fs/list", nil)))
	scopeKey := buildDirScopeKey("/user_storage", authHash)
	err = dirStore.UpsertSnapshot(context.Background(), DirListSnapshot{
		ScopeKey:      scopeKey,
		DisplayPath:   "/user_storage",
		AuthScopeHash: authHash,
		SourceMode:    dirSyncModeReq,
		SyncState:     "fresh",
		NextRefreshAt: time.Now().Add(2 * time.Minute),
		PayloadJSON:   []byte(`{"code":200,"message":"success","data":{"content":[{"name":"user_storage","is_dir":true,"size":0,"type":1}],"total":1}}`),
	})
	if err != nil {
	...
	if got, _ := resp.Data.Content[0]["path"].(string); got != "/user_storage/正确目录" {
		t.Fatalf("path=%q, want live upstream path", got)
	}
}
...
func TestHandleFsSearchRecursiveDecryptsDisplayNames(t *testing.T) {
	...
	mux.HandleFunc("/api/fs/list", func(w http.ResponseWriter, r *http.Request) {
		...
		case "/personal_cloud/encrypt/" + rootDirRaw:
			writeJSONResponse(w, map[string]interface{}{
				"code":    200,
				"message": "success",
				"data": map[string]interface{}{
	...
	if got := resp.Data.Content[0]["path"].(string); got != "/personal_cloud/encrypt/season1/"+leafDisplay {
